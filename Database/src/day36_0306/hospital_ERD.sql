...
VALUES ('C1', '지구여행', 2000 - 02 - 01, '231001', '문지영', 2023 - 04 - 01, 'D1', '화학공학과'),
       ...
       ('C4', 'null', 2021 - 02 - 01, 'null', 'null', now(), 'D5', '인공지능학과');
...
alter table Patients
    add constraint pat_doc_id_fk FOREIGN KEY (doc_id) REFERENCES Doctors (doc_id);

alter table Patients
    add constraint pat_nur_id_fk FOREIGN KEY (nur_id) REFERENCES Nurses(nur_id);
...
alter table Treatments
    add constraint treat_pat_id_fk FOREIGN KEY (pat_id) REFERENCES Patients (pat_id);

alter table Treatments
    add constraint treat_doc_id_fk FOREIGN KEY (doc_id) REFERENCES Doctors(doc_id);
...
alter table Charts
    add constraint charts_treat_doc_pat_id_pk PRIMARY KEY (chart_id,treat_id,pat_id,doc_id);

alter table Charts
    add constraint charts_nur_id_fk FOREIGN KEY (nur_id) REFERENCES Nurses (nur_id);
